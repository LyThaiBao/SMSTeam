  ...
  return (
    <li className="bg-[#eef1f2e1] p-4 mb-3 flex flex-col gap-3 md:flex-row justify-between items-center rounded-2xl shadow-sm border border-slate-100 text-black font-thin transition">
      ...
      <div className="flex justify-between items-center w-full md:max-w-[45%]">
        <button
          onClick={() => onDetail(id)}
          className="cursor-pointer bg-[#333] text-white border rounded-2xl font-bold py-4 px-5 hover:border-blue-500"
        >
          Xem Chi Tiết
        </button>
        <button
          className="cursor-pointer bg-[#333] py-4 px-5 rounded-2xl font-bold text-white"
          onClick={() => onDelete(id)}
        >
          Xóa
        </button>
