...
export default function FormAddDepartment() {
  ...
  async function onSubmit(data: FormDepartmentType) {
    // Logic gửi dữ liệu lên server (sử dụng FormData để gửi file)
  ...
  const inputStyle = "w-full p-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50";
    ...
    >
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <footer className="p-6 border-t border-slate-100 flex justify-end gap-3">
          <button type="button"  className="px-6 py-2.5 rounded-xl text-slate-600 font-semibold hover:bg-slate-100 transition-all">
            Hủy
          </button>
          <button 
            type="submit" 
